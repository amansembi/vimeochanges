...
class WP_DGV_Ajax_Handler {
	...
	public function store_upload() {
		...
		$vimeo_video_type = isset($_POST['vimeo_video_type'] ) ? $_POST['vimeo_video_type'] : 'not found';
		$uri = $_POST['uri'];
		$videoID = preg_replace( '/\D/', '', html_entity_decode($uri) );
		$result = $this->db_helper->create_local_video( $title, $description, $uri, $vimeo_video_type, $videoID);
